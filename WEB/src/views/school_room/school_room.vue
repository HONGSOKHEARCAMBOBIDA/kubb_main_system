...
const columns = [
  ...
  { prop: 'capacity', label: 'ចំណុះ', width: 100 },
...
const rules = {
  ...
  name: [{ required: true, message: 'សូមបញ្ចូលឈ្មោះបន្ទប់', trigger: 'blur' }],
  capacity: [{ required: true, message: 'សូមបញ្ចូលចំណុះ', trigger: 'blur' }],
...
async function fetchSchoolRooms() {
  ...
  try {
    ...
    schoolRooms.value = res.data.data || []
...
function openCreate() {
  ...
  dialogTitle.value = 'បង្កើតបន្ទប់ថ្មី'
  form.floor_id = null
  form.code = ''
  form.name = ''
  form.capacity = null
...
function openEdit(row) {
  ...
  dialogTitle.value = 'កែប្រែបន្ទប់'
    ...
    >
      <template #search>
        <AppInput
          ...
          placeholder="ស្វែងរកតាមឈ្មោះបន្ទប់"
          clearable
