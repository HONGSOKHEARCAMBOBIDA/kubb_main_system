...
import { CreateClassRepresentative } from '../../services/class_representative.service.js'
import {
...
// updateclasscurriculumndetail
const updateclasscurriculumndetailsemesteroption = ref([])
const updateclasscurriculumndetailshiftoption = ref([])
const updateclasscurricolumndetailuuid = ref(null)
const updateclasscurricolumndetailacademicid = ref(null)
const updateclasscurricolumndetailform = reactive({
  semester_id: null,
  study_year_id: null,
...
const updateclasscurricolumndetailvisible = ref(false)

async function openupdateclasscurricolumndetail(row) {
  updateclasscurricolumndetailuuid.value = row.uuid
  updateclasscurricolumndetailform.study_year_id = row.study_year_id
  updateclasscurricolumndetailform.midterm_date = row.midterm_date
  updateclasscurricolumndetailform.final_date = row.final_date
  updateclasscurricolumndetailform.type_class = row.type_class
  updateclasscurricolumndetailform.start_date = row.start_date
  if (row.academic_id) {
    updateclasscurricolumndetailacademicid.value = row.academic_id
    updateclasscurriculumndetailsemesteroption.value = await loadSemesterOptions(row.academic_id)
    updateclasscurriculumndetailshiftoption.value = await loadAcademicShiftOptions(row.academic_id)
  }
  updateclasscurricolumndetailform.semester_id = row.semester_id ?? null
  updateclasscurricolumndetailform.academic_shift_id = row.academic_shift_id ?? null
  updateclasscurricolumndetailvisible.value = true
}

async function updateclasscurricolumndetailacademicidchange(
  updateclasscurricolumndetailacademicid
) {
  updateclasscurriculumndetailsemesteroption.value = []
  updateclasscurriculumndetailshiftoption.value = []
  updateclasscurricolumndetailform.semester_id = null
  updateclasscurricolumndetailform.academic_shift_id = null
  updateclasscurriculumndetailsemesteroption.value = await loadSemesterOptions(
    updateclasscurricolumndetailacademicid
  )
  updateclasscurriculumndetailshiftoption.value = await loadAcademicShiftOptions(
    updateclasscurricolumndetailacademicid
  )
}

async function handleSubmitUpdateClassCurriculumDetail() {
  submitting.value = true
  try {
    await UpdateClassCurriculumnDetail(
      updateclasscurricolumndetailuuid.value,
      updateclasscurricolumndetailform
    )
    notify.success('កែប្រែកម្មវិធីសិក្សាបានជោគជ័យ')
    updateclasscurricolumndetailvisible.value = false
    fetchClassCurriculum()
  } catch (e) {
    notify.error(e?.response?.data?.message || e.message || 'Failed to create')
  } finally {
...
async function submitclassrepresentative() {
  if (!selectclasscurriculumndetail.value?.id) {
    notify.error('Class curriculum detail is required')
    return
  }

  if (selectrowstudentterms.value.length === 0) {
...
const columnclass_offering = [
  ...
  {
    prop: 'total_attendance_for_rexam',
    slot: 'total_attendance_for_rexam',
    label: 'អវត្តមានប្រឡងសង',
    minwidth: 120,
  },
  {
    prop: 'total_attendance_for_relearn',
    slot: 'total_attendance_for_relearn',
    label: 'អវត្តមានរៀនសង',
    minwidth: 120,
  },
...
async function openClassRegistrationDialog(detailRow, curriculumRow, class_offering) {
  classregistrationfilters.semester_id = detailRow.semester_id
  classregistrationfilters.study_year_id = detailRow.study_year_id
  classregistrationfilters.term_id = curriculumRow.term_id
  classregistrationfilters.major_id = curriculumRow.major_id
...
async function submitcourseregistration() {
  ...
  try {
    ...
    notify.error(e?.response?.data?.message || e.message || 'Failed to create course registration')
  }
}
...
<template>
  ...
  >
    ...
    <template #major_name="{ row }">
      <el-text>
        {{ row.major_code }} | <el-text size="small" type="primary">{{ row.major_name }}</el-text> |
        <el-text size="small">{{ row.faculty_name }}</el-text>
      </el-text>
    </template>
  ...
  <template #class_representative="{ row }">
    ...
    <el-text v-else type="info">មិនទាន់មានប្រធានថ្នាក់</el-text>
        ...
        <template #actions="{ row: detailRow }">
          ...
          <el-tooltip content="កែប្រែ" placement="top">
            <AppButton
              @click="openupdateclasscurricolumndetail(detailRow)"
              plain
              type="warning"
              icon="Edit"
              circle
          ...
          <el-tooltip content="រើសប្រធានថ្នាក់" placement="top">
            <AppButton
              @click="openclasrepresentativeDialog(detailRow, row)"
              plain
              type="success"
              icon="UserFilled"
              circle
        ...
        <template #expand="{ row: detailRow }">
          <el-divider content-position="left">
            <el-text tag="b" style="color: darkcyan"> មុខវិជ្ជាត្រូវសិក្សា </el-text>
          </el-divider>
          <TableCustom
            expandable
          ...
          >
            ...
            <template #total_hour="{ row }">
              <el-text> {{ row.total_hour }} ម៉ោង </el-text>
            </template>
            ...
            <template #expand="{ row: offeringRow }">
              <el-divider content-position="left">
                <el-text tag="b" style="color: darkcyan"
                  >សិស្សសរុបកំពុងសិក្សា {{ offeringRow.student.length }} នាក់</el-text
                >
              </el-divider>
              <TableCustom
                expandable
              ...
              >
                ...
                <template #expand="{ row: studentgrade }">
                  <el-divider> Student Grade </el-divider>
                  <TableCustom
                    expandable
          ...
          <el-row :gutter="20">
            <el-col :span="6">
              <AppInput
                ...
                type="date"
                placeholder="ថ្ងៃប្រឡងចាប់ផ្កើម"
                clearable
                label="ថ្ងៃប្រឡងចាប់ផ្កើម"
              />
              <AppInput
  ...
  <AppDialog
    v-if="updateclasscurricolumndetailvisible"
    v-model:visible="updateclasscurricolumndetailvisible"
    title="កែប្រែកម្មវិធីសិក្សាថ្មី"
    :showDefaultFooter="false"
    width="60%"
  >
    <AppForm
      ref="formRef"
      :model="updateclasscurricolumndetailform"
      :loading="submitting"
      :show-actions="true"
      @submit="handleSubmitUpdateClassCurriculumDetail"
      submitText="រក្សាទុក"
      resetText="ចាកចេញ"
    >
      <!-- detail rows -->
      <div class="detail-section">
        <div class="detail-row">
          <el-row :gutter="20">
            <el-col :span="12">
              <AppSelect
                v-model="updateclasscurricolumndetailacademicid"
                :options="academicOptions"
                placeholder="ជ្រើសរើសឆ្នាំសិក្សា"
                clearable
                label="ឆ្នាំសិក្សា"
                @change="
                  updateclasscurricolumndetailacademicidchange(
                    updateclasscurricolumndetailacademicid
                  )
                "
              />
            ...
            <el-col :span="12">
              <AppSelect
                v-model="updateclasscurricolumndetailform.study_year_id"
                :options="studyyearOption"
                placeholder="ឆ្នាំទី"
                clearable
                label="ឆ្នាំទី"
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <AppSelect
                v-model="updateclasscurricolumndetailform.semester_id"
                :options="updateclasscurriculumndetailsemesteroption"
                placeholder="ជ្រើសរើសឆមាស"
                clearable
                label="ឆមាស"
                :disabled="!updateclasscurricolumndetailacademicid"
              />
            </el-col>
            <el-col :span="12">
              <AppSelect
                v-model="updateclasscurricolumndetailform.academic_shift_id"
                :options="updateclasscurriculumndetailshiftoption"
                placeholder="ជ្រើសរើសវេន"
                clearable
                label="វេន"
                :disabled="!updateclasscurricolumndetailacademicid"
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <AppInput
                v-model="updateclasscurricolumndetailform.start_date"
                type="date"
            ...
            <el-col :span="12">
              <AppInput
                v-model="updateclasscurricolumndetailform.midterm_date"
                type="date"
                placeholder="ថ្ងៃប្រឡង Midterm"
                clearable
                label="ថ្ងៃប្រឡង Midterm"
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <AppInput
                v-model="updateclasscurricolumndetailform.final_date"
                type="date"
                placeholder="ថ្ងៃប្រឡង Final"
                clearable
                label="ថ្ងៃប្រឡង Final"
            ...
            <el-col :span="12">
              <AppSelect
                v-model="updateclasscurricolumndetailform.type_class"
                :options="typeClassOptions"
                placeholder="ជ្រើសរើសប្រភេទថ្នាក់"
                clearable
                label="ប្រភេទថ្នាក់"
